...
//   return ((selected.attributes.value || {}).specified ?
//       selected.value :
//       selected.text);
// }
...
//   setHtml(id, (count === '' ?
//       '' :
//       '(' + (count + '').replace(/\B(?=(\d{3})+$)/g, thousandsSeparator) +
//       ')'));
//   var el = qs('#' + id);
//   if (el) {
...
//   if (isTag(a, 'a|span') && a.offsetLeft + a.offsetWidth >
//       a.parentNode.offsetWidth - 15) {
...
//       isTag(target, 'select|textarea|input')) {
//     target.blur();
//     if (button) {
...
//   var target = getTarget(event);
//   if ((isCtrl(event) || event.shiftKey) && target.type == 'submit' &&
//       isTag(target, 'input')) {
...
//   if ((event.keyCode == 40 || event.keyCode == 38) && isCtrl(event)) {
//     var target = getTarget(event);
//     var sibling = (event.keyCode == 40 ? 'nextSibling' : 'previousSibling');
//     var el = target.parentNode.parentNode[sibling];
//     if (el && (isTag(el, 'tr') || (el = el[sibling])) && isTag(el, 'tr') &&
//         (el = el.childNodes[nodePosition(target.parentNode)]) &&
//         (el = el.childNodes[nodePosition(target)])) {
//       el.focus();
//     }
//     return false;
//   }
...
//               '<div class="error">' + offlineMessage + '</div>');
//           ajaxStatus.className = ajaxStatus.className.replace(/ hidden/g, '');
//         }
//       }
//     };
...
//         return false;
//       }
//       if (!/^(checkbox|radio|submit|file)$/i.test(el.type) || el.checked ||
//           el == button) {
//         data.push(encodeURIComponent(el.name) + '=' + encodeURIComponent(
//             isTag(el, 'select') ? selectValue(el) : el.value));
//       }
//     }
//   }
//   data = data.join('&');
...
//   var td = this;
//   var target = getTarget(event);
//   if (!isCtrl(event) || isTag(td.firstChild, 'input|textarea') ||
//       isTag(target, 'a')) {
//     return;
//   }
//   if (warning) {
...
//       event = window.event;
//     }
//     if (event.keyCode == 27 && !event.shiftKey && !event.altKey &&
//         !isCtrl(event)) {
...
//   var value = (td.firstChild && td.firstChild.alt) || td.textContent ||
//       td.innerText;
//   input.style.width = Math.max(td.clientWidth - 14, 20) + 'px';
//   if (text) {
//     var rows = 1;
//     value.replace(/\n/g, function() {
//       rows++;
...
//   input.focus();
//   if (text == 2) {
//     return ajax(location.href + '&' + encodeURIComponent(td.id) + '=',
//         function(request) {
...
//   alterClass(target, 'maxlength',
//       target.value && maxLength != null && target.value.length > maxLength);
// };
...
//         }
//         obj[key] = (maria ?
//             obj[key].replace(/dev\.mysql\.com\/doc\/mysql\/en\//,
//                 'mariadb.com/kb/en/library/') :
//             obj[key]).replace(/\/doc\/mysql/, '/doc/refman/' + version).
...
//         if (col in indexColumns && (!/LIKE|REGEXP/.test(op) ||
//             (op == 'LIKE' && val.charAt(0) != '%'))) {
//           return true;
//         } else if (col || val) {
//           ok = false;
//         }
...
//   return (val == a + '_' + b || val == a + b || val == a +
...
//       }, onchange: editingTypeChange, onmouseover: function(event) {
...
//     if (!match) {
//       if (candidate && i == opts.length - 2 && val ==
//           opts[candidate].value.replace(/.+`/, '') && name == 'fields[1]') {
//         return;
//       }
//       break;
//     }
...
//       table = tables[j];
//       if (val == column || val == table || delimiterEqual(val, table, column) ||
//           delimiterEqual(val, column, table)) {
//         if (candidate) {
//           return;
//         }
//         candidate = i;
...
//       if (!((/(char|binary)$/.test(lastType) && /(char|binary)$/.test(text)) ||
//           (/(enum|set)$/.test(lastType) && /(enum|set)$/.test(text)))) {
//         el.value = '';
//       }
//       el.oninput.apply(el);
//     }
//     if (lastType == 'timestamp' && el.name == name + '[has_default]' &&
//         /timestamp/i.test(formField(type.form, name + '[default]').value)) {
//       el.checked = false;
//     }
...
//           !/(^|[^o])int(?!er)|numeric|real|float|double|decimal|money/.test(
//               text));
//     }
//     if (el.name == name + '[on_update]') {
//       alterClass(el, 'hidden', !/timestamp|datetime/.test(text));
//     }
...
//   alterClass(this, 'required', !this.value.length && /var(char|binary)$/.test(
//       selectValue(this.parentNode.previousSibling.firstChild)));
// }
...
//   alterClass(this.form['partitions'], 'hidden',
//       partitionTable || !this.selectedIndex);
...
//     }
//   }
//   this.form[this.name.replace(/\].*/, '][name]')].value = prefix +
//       names.join('_');
// }
...
//   if (!select.selectedIndex) {
//     while (selectValue(select) != 'INDEX' && select.selectedIndex <
...
//     form['Trigger'].value = table + '_' +
//         (selectValue(form['Timing']).charAt(0) +
//             formEvent.charAt(0)).toLowerCase();
//   }
...
//         var div2 = qs('[id="' + (/^refs/.test(divs[i].id) ? 'refd' : 'refs') +
//             divs[i].id.substr(4) + '"]');
//         var ref = (tablePos[divs[i].title] ?
//             tablePos[divs[i].title] :
//             [div2.parentNode.offsetTop / em, 0]);
//         var left1 = -1;
...
//         if (!lineSet[id]) {
//           var line = qs(
//               '[id="' + divs[i].id.replace(/^....(.+)-.+$/, 'refl$1') + '"]');
//           var top1 = top + divs[i].offsetTop / em;
//           var top2 = top + div2.offsetTop / em;
//           if (divs[i].parentNode != div2.parentNode) {
//             top2 += ref[0] - top;
//             line.querySelector('div').style.height = Math.abs(top1 - top2) +
//                 'em';
//           }
...
//   if (that !== undefined) {
//     tablePos[that.firstChild.firstChild.firstChild.data] = [
//       (event.clientY - y) / em, (event.clientX - x) / em];
//     that = undefined;
//     var s = '';
//     for (var key in tablePos) {
//       s += '_' + key + ':' + Math.round(tablePos[key][0] * 10000) / 10000 +
//           'x' + Math.round(tablePos[key][1] * 10000) / 10000;
//     }
//     s = encodeURIComponent(s.substr(1));
//     var link = qs('#schema-link');
//     link.href = link.href.replace(/[^=]+$/, '') + s;
//     cookie('adminer_schema-' + db + '=' + s, 30);
//   }
// }

// var helpOpen, helpIgnore;
...
//     help.style.top = (body.scrollTop + rect.top - (side ?
//         (help.offsetHeight - target.offsetHeight) / 2 :
//         help.offsetHeight)) + 'px';
//     help.style.left = (body.scrollLeft + rect.left - (side ?
//         help.offsetWidth :
//         (help.offsetWidth - target.offsetWidth) / 2)) + 'px';
//   }
// }
